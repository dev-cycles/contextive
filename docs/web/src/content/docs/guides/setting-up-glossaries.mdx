...
If you need terms from another glossary file, the [import](#terms-relevant-in-multiple-places) facility may assist.
:::
...
:::note
For this to work, you will need to build your own processes (manual or automated) to extract the terminology definitions from the external dataset and map them to a Contextive glossary yml format.
:::
...
With this setup, any terminology contained in the file located at `https://your.domain/path/to/shipping.glossary.yml` will be made available as though it had been defined in `shipping/shipping.glossary.yml`.
...
:::caution[No Refresh]
Remote glossary files are loaded once at startup and there is not yet any way to force a reload other than restarting your IDE
:::
...
Some organisations have adopted a distributed repository configuration, where small services (microservices, or cloud FAAS functions) are each located in their own repositories. Often in these cases, the code in the repositories share a domain language.
