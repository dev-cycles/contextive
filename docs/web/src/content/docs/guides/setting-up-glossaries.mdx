...
You can have as many glossary files as you like in your repository. For Contextive to find a glossary file, it must match the pattern `*.glossary.yml`. You can name the prefix whatever you like, so `project.glossary.yml` and `module.glossary.yml` are both valid filenames, but `module-glossary.yml` is not.
...
2. Decide where to put it - this affects which files are supported by the terminology in that glossary file - see [Glossary Location](#glossary-location)
