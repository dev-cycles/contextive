namespace Contextive.Cloud.Api.Tests
...
module HttpHandlersTests =
    ...
    let ``Request HTTP Get at /``() = async {
        ...
        let request = JsonConvert.DeserializeObject<APIGatewayHttpApiV2ProxyRequest>(requestStr)
        let context = TestLambdaContext()
        let! response = lambdaFunction.FunctionHandlerAsync(request, context) |> Async.AwaitTask

        Assert.Equal(200, response.StatusCode)
        Assert.StartsWith("Serverless Giraffe Web API", response.Body)
        Assert.True(response.Headers.ContainsKey("Content-Type"))
        Assert.Contains("text/plain", response.Headers.Item("Content-Type"))
    }
