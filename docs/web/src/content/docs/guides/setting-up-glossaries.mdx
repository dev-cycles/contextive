...
When getting started, it could be helpful to copy the <Link href="https://github.com/dev-cycles/contextive/blob/v{contextive_version}/src/language-server/Contextive.LanguageServer.Tests/E2e/GlossaryFileInitializationTests.Default%20Glossary%20File.verified.txt">default glossary file template</Link> which contains all possible fields and is used to define the terms used in the glossary file itself.
