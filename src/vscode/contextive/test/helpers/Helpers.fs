...
let findUriInVisibleEditors (docUri: Uri) =
    window.visibleTextEditors.Find(fun te ->
        ...
        // On windows, the drive letter was capitalised in the docUri, not in the textEditor
...
[<Emit("afterEach($0, $1)")>]
let afterEach (_: string, _: unit -> JS.Promise<unit>) : unit = jsNative

[<Emit("after($0, $1)")>]
let after (_: string, _: unit -> JS.Promise<unit>) : unit = jsNative

[<Emit("beforeEach($0, $1)")>]
let beforeEach (_: string, _: unit -> JS.Promise<unit>) : TestCase = jsNative

[<Emit("before($0, $1)")>]
let before (_: string, _: unit -> JS.Promise<unit>) : TestCase = jsNative
