module Contextly.LanguageServer.Tests.HoverTests
...
let hoverTests =
    testList "Hover Tests" [
        testAsync "Given no contextly and no document sync, server response to hover request with empty result" {
        ...
        let testHoverTermNotFound (text, position: Position) = 
            testAsync $"Given contextly and document open, server responds to hover request with no content in {position}" {
                let fileName = "one"
                let config = [
                        Workspace.optionsBuilder <| Path.Combine("fixtures", "completion_tests")
                        ConfigurationSection.contextlyPathOptionsBuilder $"{fileName}.yml"
