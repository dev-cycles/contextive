...
type Lexer = 
    ...
    static member private startOfToken line position = Start(line, line.LastIndexOfAny(Lexer.delimiters, position) + 1)

    static member ofLine (lines:IList<string>) =
        function | lineNumber when lineNumber >= lines.Count -> NoToken
