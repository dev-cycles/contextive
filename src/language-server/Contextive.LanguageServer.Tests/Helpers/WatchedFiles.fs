...
let optionsBuilder registrationAwaiter (b: LanguageClientOptions) =
    ...
    b
        ...
        .OnUnregisterCapability(fun p ->
            let reg =
                p.Unregistrations
                |> Seq.tryFind (fun r -> r.Method = WorkspaceNames.DidChangeWatchedFiles)

            match reg with
            | Some r -> Unregistered(r.Id) |> received
            | _ -> ()
        ...
        .OnRegisterCapability(fun (p: RegistrationParams) ->
            ...
            | Some r ->
                let opts =
                    (r.RegisterOptions :?> JToken)
                        .ToObject<DidChangeWatchedFilesRegistrationOptions>()
