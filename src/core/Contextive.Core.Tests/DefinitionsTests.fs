...
let definitionsTests =
    testList "Core.Definitions Tests" [
        ...
        testCase "Context has default" <| fun () ->
        ...
        testCase "Definitions has default" <| fun () ->
            let definitions = Definitions.Default
            test <@ definitions.Contexts.Count = 0 @>

        testCase "Context with new terms has new terms" <| fun () ->
            let context = Context.Default.WithTerms(seq{ Term.Default })
            test <@ context.Terms.Count = 1 @>
            test <@ context.Terms[0] = Term.Default @>

        testList "Successful Parsing" [

            testCase "Minimal parse has parse defaults" <| fun () ->
                let definitions = unwrap <| deserialize """
contexts:
  - terms:
    - name: firstTerm
"""
            ...
            testCase "Can Parse Term Name" <| fun () -> 
                let definitions = unwrap <| deserialize """
contexts:
  - terms:
    - name: firstTerm
"""
            ...
            testCase "Can Parse Term Definition" <| fun () ->
                let definitions = unwrap <| deserialize """
contexts:
  - terms:
    - name: firstTerm
...
"""
            ...
            testCase "Can Parse Term Examples" <| fun () -> 
                let definitions = unwrap <| deserialize """
contexts:
  - terms:
    - name: firstTerm
...
"""
            ...
            testCase "Can Parse Context Name" <| fun () -> 
                let definitions = unwrap <| deserialize """
contexts:
  - name: The Context
"""
            ...
            testCase "Can Parse Context Domain Vision Statement" <| fun () -> 
                let definitions = unwrap <| deserialize """
contexts:
...
"""
            ...
            testCase "Can Parse Context Paths" <| fun () -> 
                let definitions = unwrap <| deserialize """
contexts:
  - paths:
    - "path1"
...
"""
        ...
        testList "Parsing Errors" [

            testCase "Error when file is empty" <| fun () ->
                let errorMessage = unwrapError <| deserialize ""
                test <@ errorMessage = "Definitions file is empty." @>

            testCase "Error with unexpected key" <| fun () ->
                let errorMessage = unwrapError <| deserialize """
unknown:
"""
                test <@ errorMessage = "Error parsing definitions file:  Object starting line 2, column 1 - Property 'unknown' not found on type 'Contextive.Core.Definitions+Definitions'." @>

            testCase "Error with extra colon typo" <| fun () ->
                let errorMessage = unwrapError <| deserialize """
contexts:
  - terms:
    - name: invalidTerm
...
"""
                test <@ errorMessage = "Error parsing definitions file:  Object starting line 5, column 19 - Mapping values are not allowed in this context." @>
        ]
    ]